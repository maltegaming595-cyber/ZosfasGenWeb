// Shareable UX patch: role-based navigation (member vs admin)
async function __zosfas_applyRoleNav() {
  try {
    const r = await fetch("/api/admin/check");
    if (!r.ok) return;
    const j = await r.json();
    if (j && j.ok && j.admin) document.body.classList.add("is-admin");
    else document.body.classList.add("is-member");
  } catch {
    document.body.classList.add("is-member");
  }
}
document.addEventListener("DOMContentLoaded", __zosfas_applyRoleNav);
(() => {
...
const routes = {
  ...
  function routeFromHash(){
    ...
    // Member-first UX: block admin-only routes unless the user has the admin role.
    const adminRoutes = new Set(["codes","admin","settings"]);
    const isAdmin = document.body.classList.contains("is-admin");
    if (adminRoutes.has(route) && !isAdmin) return "generator";
...
// Mobile nav toggle (if sidebar exists)
document.addEventListener("DOMContentLoaded", () => {
  const navBtn = document.getElementById("navToggle");
  if (navBtn) {
    // show on small screens
    const mq = window.matchMedia("(max-width: 860px)");
    const apply = () => { navBtn.style.display = mq.matches ? "inline-flex" : "none"; if(!mq.matches) document.body.classList.remove("nav-open"); };
  ...
  const t = document.getElementById("siteLinkText");
  if (t) t.textContent = siteUrl;

  const copyBtn = document.getElementById("copySiteLink");
  if (copyBtn) copyBtn.addEventListener("click", async () => {
    try { await navigator.clipboard.writeText(siteUrl); alert("Copied!"); }
    catch { alert("Copy failed. Link: " + siteUrl); }
  });

  const shareDiscordLink = document.getElementById("shareDiscordLink");
  if (shareDiscordLink) {
    fetch("/api/public").then(r => r.json()).then(j => {
      if (j && j.discordInviteUrl) shareDiscordLink.href = j.discordInviteUrl;
    }).catch(()=>{});
  }
});
