...
const ROLE_ADMIN = process.env.ROLE_ADMIN || "1467344940383338662";
...
function memberIsAdmin(member) {
  // Admin view is only for members with the configured admin role.
  // This keeps the site member-friendly and avoids exposing admin tabs.
  return Boolean(member?.roles?.cache?.has(ROLE_ADMIN));
}
...
router.get("/public", async (req, res) => {
  // Public info for the landing page / link previews
  try {
    const discordInviteUrl = process.env.DISCORD_INVITE_URL || process.env.LINK_DISCORD || "";
    const onlineText = (process.env.ONLINE_TEXT || "online");
    return res.json({ ok: true, discordInviteUrl, onlineText });
  } catch {
    return res.json({ ok: true, discordInviteUrl: "", onlineText: "online" });
  }
});
